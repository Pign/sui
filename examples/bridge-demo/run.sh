...
PLATFORM="${1:-macos}"
APP_NAME="BridgeDemo"
BUNDLE_ID="com.haxeapple.bridgedemo"
...
# Collect all .o files except __main__.o (has conflicting main() symbol)
mkdir -p build/lib
find build/cpp/obj -name "*.o" ! -name "*__main__*" | xargs ar rcs build/lib/libhaxe.a

echo "==> [3/5] Compiling C++ bridge..."
# Compile the generated bridge C++ against hxcpp headers
clang++ -c \
  -std=c++17 \
  -I"$HXCPP_DIR/include" \
  -I"build/cpp/include" \
...
# Copy Swift files (not the .cpp — it's compiled into the .a)
cp build/swift/App.swift build/swift/ContentView.swift build/swift/HaxeBridgeC.swift build/$PLATFORM/Sources/
cp build/swift/HaxeBridgeC.h build/$PLATFORM/Sources/
cp build/lib/libhaxe.a build/$PLATFORM/lib/

# Always copy runtime files (ensures latest version)
cp ../../runtime/swift/HaxeBridge.swift build/$PLATFORM/Sources/
cp ../../runtime/swift/ViewBridge.swift build/$PLATFORM/Sources/
# HaxeRuntime: use the bridge-aware version since we have hxcpp linked
